...
typedef enum{
	...
	NCL_EVENT_ERROR,///< \brief Something has failed. Only possible after calling  \ref nclInit. No associated data.
...
Some components that are initialized here may fail at any time.
If this happens, an \ref NCL_EVENT_ERROR is triggered.
This allows the \ref NEA to know a failure occurred.
The NEA can then do whatever work it needs to do, then call \ref nclFinish, so \ref nclInit may be called again, in hopes of smoothly recovering.
...
A failure is typically indicated by an NCL function returning NCL_FALSE;
...
/*! \brief Tell a Nymi to sign a message after a signature key pair has been created.
\ingroup securityFunctions

\details
This function tells a \ref Nymi to sign a message, using a signing key created with  \ref nclCreateSigKeyPair.
...
\return  \ref NCL_FALSE On failed initiation.
*/
NCL NclBool nclCreateGlobalSigKeyPair(int nymiHandle, NclPartnerPublicKey partnerPublicKey, NclSig bionymSignature);

/*! \brief Tell a Nymi detected Nymi to sign a message with a global signature key pair.
\ingroup securityFunctions

\details
This function tells a Nymi to sign a specified message, using a signing key created with  \ref nclCreateGlobalSigKeyPair.
Unlike most other NCL functions, the Nymi does not need to be connected to the  \ref NEA through  \ref nclValidate. Therefore, the Nymi does not need to have been provisioned to this  \ref NEA.

\pre NCL is initialized. The specified Nymi has been recently \ref detection "detected".
\post A connection is established with the Nymi, and its \ref commandChannel "command channel" is taken up. An \ref NCL_EVENT_GLOBAL_SIG will be triggered when the Nymi signs the message, at which point the command channel is free.

\param [in] nymiHandle The \ref nymiHandle "handle" of the Nymi you want to sign a message.
\param [in] partnerPrivateKey Partner private key of the partner key pair associated with the signature key pair whose verification key should be able to verify the signature.
...
NCL NclBool nclGlobalSign(
	int nymiHandle,
	NclPartnerPrivateKey partnerPrivateKey,
	NclPartnerPublicKey partnerPublicKey,
	NclMessage message
);
...
This function does not communicate with a Nymi.
